...
![diffusers-match workflow preview](docs/diffusers_exact_workflow_preview.png)
...
- SDXL teacher CLIP 在严格对齐 workflow 里固定用 `DualCLIPLoader(device="cpu")`，避免和 diffusers single-file teacher embedding 出现微小差异。
...
- RUM transformer 的 timestep、CFG 和 guidance embedding 处理需要贴近原始推理路径。
- FLUX.2 VAE decode 需要匹配 diffusers 的 BN、unpatchify、attention 公式、BF16 postprocess 和 PIL round 量化路径。
...
- teacher CLIP 文件不一致，或者 `DualCLIPLoader` 没有按验证 workflow 使用 `device="cpu"`。
...
- The strict workflow loads SDXL teacher CLIP with `DualCLIPLoader(device="cpu")` to avoid tiny embedding differences from the diffusers single-file teacher path.
...
- Timestep, CFG, and guidance embedding behavior must follow the original inference path.
- FLUX.2 VAE decode must match diffusers BN, unpatchify, attention formula, BF16 postprocess, and PIL round quantization.
...
- Teacher CLIP files differ, or `DualCLIPLoader` is not using `device="cpu"` as in the validation workflow.
